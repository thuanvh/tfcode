...
class TextDataFileList:
  def __init__(self, listfilename, wdict, batch_size, loop = True, epoch_changed=None):
  ...
  def get_next_batch(self):
    ...
    while len(batch_x) < self.batch_size :
      ...
      if len(batch_x) == 0:
        label, data = process.binarize_samples_lines(self.cur_text[start:end], self.wdict)
  ...
  def get_valid_set(self, valid_count):
    ...
    label, data = process.binarize_samples_lines(rand_text, self.wdict)
    batch_x = np.array(data) #self.cur_data[start:end,:]
    batch_y = np.array(label) #self.cur_label[start:end,self.y_slice]
    return batch_x, batch_y
  ...
  def get_all(self):
    ...
    for fname in self.file_list:
      ...
      #data = f.get('data')
      label,data = process.binarize_samples_files(fname, self.wdict)
