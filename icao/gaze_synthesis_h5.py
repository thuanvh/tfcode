...
output_folder = "D:/sandbox/vmakeup/src/learncnn/model_face/model29_headpose/_data/gazesynthesis_" + str(inputsize) + "_" + str(base_name) + "/"
h5_name = ["combine"]
...
def sample_gen(rangename,start_idx, end_idx):
    ...
    for cur_idx in range(start_idx, end_idx):
        ...
        if len(sample.haze) > 0 and sample.haze[0] > 0:
          img = img * (1 - sample.haze[1]) + sample.haze[0] * sample.haze[1]
