...
def gen_sample_list(line_list, sample_number_list):
  file_idx = 0
  sample_list=[]
  for line in line_list:
    ...
    if one_rect != None:
      ...
      for i in range(sample_number_list[label]):  
        sample = TrainSample()
...
def count_class(sample_list, class_count):
  count_dict = dict()
  for i in range(0,class_count):
    count_dict[i] = 0
  for s in sample_list:
    count_dict[s.label] += 1
  print("counting")
  for i in range(0,class_count):
    print("Class ", i, " : " , count_dict[i])

biwi_sample_file = "tmp/headwear.npz"
if not os.path.exists(biwi_sample_file):
  sample_number_list = [5,20]
  sample_list = gen_sample_list(line_list, sample_number_list)
  count_class(sample_list, 2)
