...
def cnn_model_fn_100(x):
  ...
  #logits = tf.layers.dense(inputs=dense, units=32)
  return logits, keep_prob
def cnn_model_fn_40(x, label_length):
  """Model function for CNN."""    
  conv1 = tf.layers.conv2d(inputs=x,filters=20,kernel_size=[3, 3],padding="same",activation=tf.nn.relu)
  pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)
  conv2 = tf.layers.conv2d(inputs=pool1,filters=40,kernel_size=[3, 3],padding="same",activation=tf.nn.relu)
  ...
  dropout = tf.layers.dropout(inputs=dense, rate=keep_prob)
  logits = tf.layers.dense(inputs=dropout, units=label_length, name="logits")
  #logits = tf.layers.dense(inputs=dense, units=32)
  return logits, keep_prob
...
def cnn_model_fn(x, input_size, label_length, is_binned_label, bins):
  if not is_binned_label:
    if input_size == 40:
      return cnn_model_fn_40(x,label_length)
    elif input_size == 100:
      return cnn_model_fn_100(x,label_length)
  else:
...
def main(_):
  ...
  dump = range(1000)
  label_length = len(dump[label_slice])
  ...
  #y_conv = deepnn(x)
  y_conv, keep_prob = cnn_model_fn(x, input_size, label_length, is_binned_label, bins)

  with tf.name_scope('loss'):
    if is_binned_label:
      #lossfn = tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_,logits=y_conv)
      #lossfn = tf.reduce_mean(lossfn)
      lossfn = tf.losses.sparse_softmax_cross_entropy(labels=y_, logits=y_conv)
    else:
  ...
  if is_binned_label:
    predictions = {
      ...
      "classes": tf.argmax(input=y_conv, axis=1),
      # Add `softmax_tensor` to the graph. It is used for PREDICT and by the
      # `logging_hook`.
      "probabilities": tf.nn.softmax(y_conv, name="softmax_tensor")
    }
    accuracy = tf.metrics.accuracy(labels=y_, predictions=predictions["classes"])
  ...
  with tf.Session() as sess:
    ...
    for i in range(max_iter):
      ...
      if is_binned_label :
        ...
        #print(batch_y)
        batch_y = tf.cast(batch_y, tf.int64).eval()
        #batch_y = tf.one_hot(indices=batch_y, depth=len(bins)).eval()
        #print(batch_y.shape)
        #batch_y = np.reshape(batch_y, (batch_y.shape[0]))
        #print(batch_y.shape)
        #print(batch_y)
      #print(batch_x.shape)
      #print(batch_y.shape)
      if i % info_iter == 0 and i > 0:
        #print('step ', i)
        if is_binned_label :
          #train_loss = accuracy.eval(feed_dict={x: batch_x, y_: batch_y})
          #print('step %d, accuracy %g' % (i, train_loss))
          #sess.run([accuracy], feed_dict={x: batch_x, y_: batch_y, keep_prob: 0.5})
          #eval_logits = y_conv.eval(feed_dict={x: batch_x, y_: batch_y})
          # eval_predict = tf.nn.softmax(y_conv).eval(feed_dict={x: batch_x, y_: batch_y})
          # batch_y_eval = tf.cast(batch_y, tf.int64)
          # batch_y_eval = tf.one_hot(indices=batch_y_eval, depth=len(bins)).eval()
          # batch_y_eval = np.reshape(batch_y_eval, (batch_y_eval.shape[0], len(bins)))
          # #accuracy,_ = tf.metrics.accuracy(labels=batch_y_eval, predictions=eval_predict)
          # #accuracy,_ = tf.metrics.accuracy(labels=batch_y, predictions=predictions["classes"])
          # tf.local_variables_initializer().run()
          train_loss = sess.run([accuracy])
          print('step', i, ' accuracy ', train_loss )
        else:
