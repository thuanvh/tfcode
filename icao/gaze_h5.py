...
output_folder = "D:/sandbox/vmakeup/src/learncnn/model_face/model29_headpose/_data/gaze_" + str(inputsize) + "_" + str(base_name) + "/"
h5_name = ["combine"]
...
def sample_gen(rangename,start_idx, end_idx):
    ...
    for cur_idx in range(start_idx, end_idx):
        ...
        #Blur
        if sample.blur > 1:
          img = cv2.blur(img, (sample.blur, sample.blur))
